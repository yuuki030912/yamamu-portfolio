...
import { buildCatalogEntry, buildDraft, GAMES } from "../scripts/new-video-article-draft.mjs";
...
test("イナイレVの新着動画を記事下書きとカタログへ登録できる", () => {
  const video = {
    id: "inazuma-test",
    title: "【イナイレV】初心者向けBASARAビルドと対戦方法を解説",
    description: "動画内でBASARAビルドを実戦解説します。",
    published: "2026-07-22T01:23:45Z",
  };
  const cfg = GAMES.inazuma;
  assert.equal(cfg.match(video.title), true);
  const draft = buildDraft(cfg, video);
  assert.match(draft, /video-inazuma-inazuma-test\.html/);
  assert.match(draft, /イナイレV攻略Wiki/);
  assert.match(draft, /meta name="robots" content="noindex"/);
  const entry = buildCatalogEntry(cfg, video);
  assert.equal(entry.category, "inazuma");
  assert.equal(entry.draft, true);
  assert.ok(entry.tags.includes("BASARA"));
  assert.ok(entry.tags.includes("初心者"));
});
